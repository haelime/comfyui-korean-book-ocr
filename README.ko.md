...
3. Queue를 한 번 실행하면 `OCR 텍스트 수정 → 이미지`의 편집 칸에 인식 결과가 자동 입력됩니다.
...
각주 정의는 본문에서 제거되고 이미지 아래에 더 작은 붉은 손글씨로 표시됩니다. 같은 노드의 `comment` 입력은 특정 밑줄에 연결하지 않는 일반 메모로 사용할 수 있습니다.

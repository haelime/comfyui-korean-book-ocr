...
1. `사진 폴더 선택…` 버튼을 누르고 Windows 폴더 창에서 사진들이 들어 있는 폴더 자체를 선택합니다. 하위 폴더도 인식합니다.
2. `텍스트 저장 폴더 선택…`으로 저장 위치를 고른 뒤 `① 사진 폴더 → OCR 텍스트 파일`을 실행하면 사진과 같은 이름의 TXT가 생성됩니다.
3. TXT를 메모장 등에서 직접 수정하고 꾸밈 문법을 넣습니다.
4. TXT 폴더 경로는 ① 노드에서 ② 노드로 연결되어 있습니다. ② 노드에서 이미지 저장 폴더만 선택한 뒤 실행하면 같은 이름의 PNG가 생성됩니다.

`기존_텍스트_보호`가 켜져 있으면 이미 존재하는 TXT는 다시 OCR하거나 덮어쓰지 않습니다. 사진 폴더의 하위 구조도 텍스트와 이미지 폴더에 그대로 유지됩니다.
사진과 TXT 저장 폴더는 ① 노드에서, 이미지 저장 폴더는 ② 노드에서 지정합니다. Windows에서는 왼쪽에 빠른 접근·바탕 화면·다운로드·드라이브가 표시되는 탐색기 방식의 폴더 선택 창이 열리므로 ComfyUI 폴더 밖의 작업 폴더도 고를 수 있습니다.
...
붉은 밑줄은 여러 가는 선, 압력 변화, 작은 끊김과 안료 입자를 겹쳐 색연필 질감으로 렌더링됩니다. 같은 문장에서는 다시 실행해도 질감 모양이 유지됩니다.

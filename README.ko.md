...
워크플로우의 `로컬 AI OCR 자동 교정` 노드는 기본적으로 보수적인 교정만 수행합니다. 사용하지 않을 때는 `자동_교정`을 끄면 원문을 그대로 통과시킵니다.

## 마스크 → 수정 및 이미지 생성

1. `Load Image`를 우클릭하고 `Open in Mask Editor`를 선택합니다.
2. OCR할 문단만 흰색으로 칠하고 저장합니다.
3. Queue를 한 번 실행하면 로컬 AI가 오인식을 교정하고 `OCR 텍스트 수정 → 이미지`의 편집 칸에 결과를 자동 입력합니다.
4. 같은 노드에서 오탈자·줄바꿈·Markdown 꾸밈·코멘트를 수정합니다.
5. 다시 Queue하면 이 노드가 수정본을 바로 이미지로 만들어 저장합니다.
6. 직접 수정한 내용을 AI 교정본으로 되돌리려면 `AI 교정본으로 재설정` 버튼을 누릅니다.

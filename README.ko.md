...
4. ② 노드에서 수정한 TXT 폴더와 이미지 저장 폴더를 각각 선택한 뒤 실행하면 같은 이름의 PNG가 생성됩니다.

`기존_텍스트_보호`가 켜져 있으면 이미 존재하는 TXT는 다시 OCR하거나 덮어쓰지 않습니다. 사진 폴더의 하위 구조도 텍스트와 이미지 폴더에 그대로 유지됩니다.
사진, 텍스트, 이미지 폴더는 각 노드의 선택 버튼으로 지정합니다. Windows에서는 탐색기 방식의 폴더 선택 창이 열리므로 ComfyUI 폴더 밖의 작업 폴더도 고를 수 있습니다.

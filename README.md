...
1. 기존 `korean_ocr_to_image` 워크플로우 아래쪽에서 `사진 폴더 선택…` 버튼을 누릅니다.
2. Windows 폴더 창에서 사진들이 들어 있는 폴더 자체를 선택하고 ① 단계를 실행합니다.
3. `텍스트 저장 폴더 선택…`으로 지정한 곳의 TXT를 직접 수정합니다.
4. TXT 폴더 경로는 ①에서 ②로 연결되어 있습니다. ②에서 이미지 저장 폴더만 고르고 실행하면 TXT마다 PNG 한 장이 생성됩니다.
